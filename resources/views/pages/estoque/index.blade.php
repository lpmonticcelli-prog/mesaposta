<x-layouts.app title="Acervo e Inventário">
    <div class="py-12 bg-gray-50 min-h-screen" x-data="estoqueApp()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6 flex justify-between items-center">
                ...
                <button @click="abrirNovo()" class="w-full sm:w-auto px-5 py-3 bg-brand-gold text-brand-dark text-sm font-extrabold rounded-md shadow-md hover:bg-brand-hover transition-colors uppercase tracking-wide flex justify-center items-center">
        ...
        <x-ui.modal alpineTitle="modoEdicao ? 'Editar Peça' : 'Nova Peça no Acervo'">
            <form :action="formAction()" method="POST" class="p-6 space-y-5">
                ...
                <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-5">
                    <button type="button" @click="aberto = false" class="px-6 py-3 bg-gray-100 text-gray-700 font-extrabold rounded-md hover:bg-gray-200 uppercase tracking-widest transition-colors text-sm">Cancelar</button>
                    <button type="submit" class="px-6 py-3 bg-brand-gold text-brand-dark font-extrabold rounded-md hover:bg-brand-hover shadow-md uppercase tracking-widest transition-colors text-sm">Salvar Alterações</button>
                </div>
            </form>
        </x-ui.modal>
    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('estoqueApp', () => ({
                aberto: false,
                modoEdicao: false,
                form: { id: '', nome: '', categoria: '', quantidade_estoque: '', valor_locacao: '', valor_reposicao: '' },
                abrirNovo() {
                ...
                formAction() {
                    return this.modoEdicao ? `{{ url('/admin/produtos') }}/${this.form.id}` : `{{ url('/admin/produtos') }}`;
                }
            }));
        });
    </script>
